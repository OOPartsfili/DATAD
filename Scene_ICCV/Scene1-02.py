...
if __name__ == '__main__':
    ...
    log_filename = 'Scene_1_02'
    carla_data_filename = 'carla_data/Scene_1_02'
    ...
    # 获取副车的位置
    Car1 = env_map.get_waypoint(main_vehicle_location).get_left_lane().next(8)[0].transform.location
    Car2 = env_map.get_waypoint(main_vehicle_location).next(25)[0].transform.location
    Car3 = env_map.get_waypoint(main_vehicle_location).get_right_lane().next(25)[0].transform.location
    Car4 = env_map.get_waypoint(main_vehicle_location).get_right_lane().next(5)[0].transform.location

    Car5 = right_vehicle_location
    ...
    vice_control5.speed_limit = 0
    vice_control5.follow_road()



    # 主车行驶300米后突发事故
    while main_vehicle.get_location().distance(TOR_location) > 20:
        sleep(0.01)

    vice_control1.speed_limit = 40
    ...
    vice_control1.speed_limit = 30
    vice_control5.speed_limit = 60


    while main_vehicle.get_location().x < TOR_location.x + 30:
        sleep(0.01)

    vice_control1.lose_control()
    threading.Thread(target=brake_throttle_retard, args=(V_Car1, -4, 0)).start()
